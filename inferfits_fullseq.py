...
def test(opt, test_dir):
  ...
  ''' iter all big-img '''
  img_name_set_full = set(map(lambda x:x.split('__')[0], os.listdir(test_dir)))
  for img_prefix in img_name_set_full:
    img_name_list = glob.glob(os.path.join(test_dir, '{}*.fits'.format(img_prefix)))
...
def run_eval(results, save_dir, img_name_list, wh=[4096,4096], thres=0.4):
  ...
  thres_score = thres
  ipd_path = os.path.join('../exp/ctdet/gfl', "{}.IPD".format(img_name_list[0].split('__')[0]))
  ...
  for subimg_name, det_img in results.items():
    filename = subimg_name.split('/')[-1].split('.')[0]
    det_img = det_img[1]
...
if __name__ == '__main__':
  ...
  ''' use train: all images '''
  test_dir = os.path.join(this_dir, '../data/Sat/tzb2Det_test/train1024/images')
