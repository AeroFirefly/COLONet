...
def test(opt, test_dir):
  ...
  run_eval_simu(results, opt.save_dir, img_name_list, thres=0.4)


def run_eval_simu(results, save_dir, img_name_list, wh=[4096,4096], thres=0.4):
  ''' notice: this code require results.json contains only 1 big-image '''
  thres_score = thres
  ipd_dir = os.path.join(save_dir, 'inferIPD')
  recreate_dir(ipd_dir)

  # # 1. check anns
  for subimg_name, det_img in results.items():
    filename = subimg_name.split('.')[0]
    ipd_path = os.path.join(ipd_dir, "{}.IPD".format(filename))
    ...
    for det_id in range(len(det_img)):
      ...
      if float(det_item[-1]) < thres_score:
        ...
        cen_x, cen_y = [st_x + width / 2, st_y + height / 2]
...
if __name__ == '__main__':
  ...
  test_dir = os.path.join(this_dir, '../data/SatSimu/simudata/test1024/images')
