...
def run_eval(results, save_dir, img_name_list, wh=[256,256], thres=0.4):
  ''' notice: this code  '''
  thres_score = thres
  ...
  for subimg_name, det_img in results.items():
    ...
    ipd_dir=os.path.join(save_dir,"test_out_ipd")
    # infer 得到的ipd存放路径
    os.makedirs(ipd_dir, exist_ok=True)
    ipd_path = os.path.join(save_dir,"test_out_ipd","{}.IPD".format(filename))
